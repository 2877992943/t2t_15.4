...
def get_feature_dict( hparams,
    ...
    inpfn=input_fn()
    # sess=tf.Session()
    # tmp=sess.run(inpfn)# inputs [batch length 1]
    # print ('')
    return inpfn

def get_input_target_vocab(hparams):
    p_hp = hparams.problem_hparams
    has_input = "inputs" in p_hp.vocabulary
    inputs_vocab_key = "inputs" if has_input else "targets"
    inputs_vocab = p_hp.vocabulary[inputs_vocab_key]
    targets_vocab = p_hp.vocabulary["targets"]
    return inputs_vocab,targets_vocab

def get_input_str2id(inputs_vocab,inputll,addEOS=True):#['xx','xxxx',,,]
    xids=inputs_vocab.encode(inputll)
    if addEOS:
        xids+=[1] #add eos
    xids = np.array([xids] * 1, dtype=np.int32)  # [batch step  ]
    xids=np.expand_dims(xids,axis=-1)
    return xids



def get_output_id2str(targets_vocab,output,skip_eos_postprocess=False):
    strs=targets_vocab.decode(_save_until_eos(
        output, skip_eos_postprocess))
    return strs
def get_output_id2str_cls(targets_vocab,output):
    return targets_vocab.decode(output)

def predict_output_id2str(result,
                         hparams,
                         decode_hp):
    p_hp = hparams.problem_hparams
    has_input = "inputs" in p_hp.vocabulary
...
def decode_from_file_seq2seq_v04(estimator,
                                 hparams,
                                decode_hp,
                                 #checkpoint_path,
                                 sorted_inputs,
    ...
    def input_fn():
        ...
        example = gen_fn()
        return _decode_input_tensor_to_features_dict(example, hparams, decode_hp)
    ######## test input func session run feature dict
    # inpfn=input_fn()
    # sess=tf.Session()
    # tmp=sess.run(inpfn)# inputs [batch length 1]
    # print (tmp)
...
def decode_interactively(estimator, hparams, decode_hp, checkpoint_path=None,inp_str=None):
