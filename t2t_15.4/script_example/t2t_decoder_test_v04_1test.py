...
def decode(estimator, hparams, decode_hp):
  ...
  input_output=decoding.decode_from_file(estimator, FLAGS.decode_from_file, hparams,
                              decode_hp, FLAGS.decode_to_file,
                              checkpoint_path=FLAGS.checkpoint_path,
                              sorted_inputs=testll, # add here
                              sorted_keys=orderDict)  # add
  return input_output
...
if __name__ == "__main__":
  testll="""非常失望，剧本完全敷衍了事，主线剧情没突破大家可以理解，可所有的人物都缺乏动机，正邪之间、妇联内部都没什么火花。团结-分裂-团结的三段式虽然老套但其实也可以利用积攒下来的形象魅力搞出意思，但剧本写得非常肤浅、平面。场面上调度混乱呆板，满屏的铁甲审美疲劳。只有笑点算得上差强人意。
""".split('\n')
  #testll=[testll[0]]+[testll[3]]
  testll=testll[:1]
  inp_ll=[]
  for line in testll:
    line=line[:int(len(line)/2)]
    inp_ll.append(line)
  orderDict=dict(zip(list(range(len(testll))),list(range(len(testll)))))
  testll=inp_ll

  tf.logging.set_verbosity(tf.logging.INFO)
  FLAGS.data_dir = './data'
  FLAGS.t2t_usr_dir = './'
  FLAGS.problem = 'class_lm_douban'
  FLAGS.corpus_dir='./corpus'
  ##
  FLAGS.model = 'transformer'
  # FLAGS.hparams_set ='transformer_tiny' #transformer_l4
